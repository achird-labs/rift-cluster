...
/// What a node does when the owner of a config key is unreachable.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, clap::ValueEnum, serde::Serialize)]
#[serde(rename_all = "kebab-case")]
pub enum DegradedMode {
    /// Refuse the write (503). The fleet stays single-writer per key.
    #[default]
    Reject,
    /// Accept the write locally and heal-merge once the owner is reachable.
    Local,
}
...
pub struct ClusterArgs {
    ...
    /// What to do with a config write whose owner is unreachable
    #[arg(
        long,
        value_name = "MODE",
        default_value = "reject",
        env = "RIFT_CLUSTER_DEGRADED_MODE"
    )]
    pub cluster_degraded_mode: DegradedMode,
