...
impl ComposedServer {
    ...
    pub async fn shutdown(self) {
        ...
        {
            tracing::warn!(error = %e, "cluster node shutdown reported an error");
        }
    }
...
pub async fn start_with_runtimes(
    ...
    let node = RaftNode::start(NodeConfig {
        ...
        data_dir: state_dir,
        secret: cluster.secret.clone(),
        routes: cluster_api::routes(Router::new(), slot.clone(), Arc::clone(&readiness)),
    })
    .await
    .context("starting the cluster control-plane node")?;
...
async fn join_or_bootstrap(node: &RaftNode, cli: &EeCli) -> anyhow::Result<()> {
    ...
    // Seeds are re-resolved on every attempt rather than at parse time: a
    // headless service's DNS record gains members as the fleet rolls, and a name
    // resolved once at startup would pin this node to whoever answered first.
    ...
    Err(anyhow::anyhow!(
        "could not join the cluster through any seed ({})",
        failures.join("; ")
    ))
}
