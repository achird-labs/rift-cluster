...
//! make sure the loopback OSS admin, which `admin_front` proxies to, is never
...
use rift_ee::seams::{AdminAuthorizer, AuthzDecision, AuthzRequest};
...
impl AdminAuthorizer for EeAuthorizer {
    fn authorize(&self, req: AuthzRequest<'_>) -> AuthzDecision {
        ...
        // Upstream's own scope header (`x-rift-scope`, not the enterprise
