...
//
// `tenancy.rs`'s unit tests cover `classify`, `Route::scope()`, `Route::action()`
// and the body parse for `PUT /admin/fleet/name`, and `fleet_session.rs` proves
// the name reaches `/_fleet/members` on the wire — but nothing called
// `dispatch()` for the write, so the line that builds the `ControlOp` was
// untested. That line has shipped wrong once before on a sibling fleet-scoped
// op: `TenantId::default()` instead of `TenantId::new(FLEET_SCOPE)`, which
// `control::validate` refuses — so the `200` below is what pins it.

/// The round trip a fleet admin can do end to end: name the fleet over real
/// HTTP, see it committed, and get refused a body that names nothing.
#[tokio::test]
async fn the_fleet_name_surface_round_trips_for_a_fleet_admin() {
    let _guard = ARGON2_COUNTER_LOCK.lock().await;
    let state = TempDir::new().expect("tempdir");
    let server = compose::start(cluster_cli(&state, &[]))
        .await
...
/// the shared front door** — and the two front-door route endpoints say so in the body they
/// answer (issue #536). Since #545 they are the only place the fact is published: the per-route
/// dispatch-count read that once carried it too is gone, so a caller not told here is not told at all.
///
...
/// ignored and every request targets `default`, so a bypass-only test would assert the default
/// tenant's `installed: true` twice and could never fail.
#[tokio::test]
async fn a_non_default_tenants_route_table_reports_not_installed_on_write_and_read() {
    ...
    // The table read is gated on `Action::ImposterRead` under the caller's tenant resolution,
    // hardcoded at its own call site in `handle`. An acme principal that does not name acme asks
    // for `default`, which it is not bound to — and RFC-002 §8.4 makes that indistinguishable
    // from the tenant not existing. Folded in from the dispatch-count test #545 removed, which
    // checked the same thing on both paths.
    let unbound = client
        .get(format!("http://{admin}/front-door/routes"))
        .header("authorization", &acme_key)
        .send()
    ...
    assert_eq!(
        ...
        404,
        "an acme principal asking for `default` is not bound to it, and RFC-002 §8.4 makes that \
         indistinguishable from the tenant not existing"
    );
