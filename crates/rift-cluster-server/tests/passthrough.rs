...
//! `--cluster` master switch off, the cluster binary composes the same server
...
fn cli(args: &[&str]) -> EeCli {
    let mut full = vec!["rift-cluster-server", "--port", "0", "--metrics-port", "0"];
...
async fn replay_loads_the_saved_imposters() {
    ...
    assert_eq!(
        rift_cluster_server::bootstrap::dispatch(&mut parsed).expect("replay dispatches"),
        rift_cluster_server::bootstrap::AfterBootstrap::Serve
    );
