...
/// bypass an observable state rather than a silent one.
///
...
// ---------------------------------------------------------------------------
// Acting-as: a principal bound to more than one tenant is narrowed to the tenant
// it names, server-side — the alternative is a server that has already sent
// another tenant's data and is trusting the client to hide it.
// ---------------------------------------------------------------------------
...
/// the acting tenant by scanning bindings for the first tenant-admin binding — the
...
/// the first match is always the right one. The imposter listing is the read
/// that shows it: each tenant sees its own port and never the other's.
#[tokio::test]
async fn a_principal_bound_to_two_tenants_sees_only_the_one_it_is_acting_as() {
    ...
    let raw_key = "rbac-key-acting-as-dual".to_owned();
    let principal = principal_with_key("acting-as-dual", &raw_key);
    let principal_id = principal.id.clone();
    ...
    // Acting as each tenant in turn must yield that tenant's imposters and no others.
    for (acting_as, own_port, other_port) in
        [("acme", 19181u64, 19182u64), ("globex", 19182, 19181)]
    {
        let response = client
        ...
        let ports: Vec<u64> = body["imposters"]
            .as_array()
            .map(|list| list.iter().filter_map(|i| i["port"].as_u64()).collect())
            .unwrap_or_default();

        assert!(
            ports.contains(&own_port),
            "acting as {acting_as}, its own imposter must be listed: {body}"
        );
        assert!(
            !ports.contains(&other_port),
            "acting as {acting_as}, the other tenant's imposter must NOT be: {body}"
        );
    }
