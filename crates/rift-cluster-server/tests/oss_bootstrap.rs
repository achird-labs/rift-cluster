//! Two core bootstrap steps that parse but must also *act* (issue #66).
//!
...
/// `rift-cluster-server` — the silent divergence this crate exists to prevent.
#[tokio::test]
async fn debug_makes_template_errors_loud() {
    ...
    let _server = Server(
        Command::new(env!("CARGO_BIN_EXE_rift-cluster-server"))
            .args([
                "--debug",
                "--configfile",
...
async fn log_flag_writes_the_logfile() {
    ...
    assert!(
        console.contains("starting RiftCluster"),
...
fn a_transient_subcommand_writes_no_pidfile() {
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_rift-cluster-server"))
        .args([
            "--pidfile",
