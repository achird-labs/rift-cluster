...
//! cluster additions. Comparing the *set of call names* rather than a
...
const EE_COMPOSE_RS: &str = "crates/rift-cluster-server/src/compose.rs";
...
fn repo_root() -> PathBuf {
    // CARGO_MANIFEST_DIR is crates/rift-cluster-server.
...
///
/// The mirror of the test above: it keeps `CLUSTER_ADDITIONS` honest, so
/// what cluster adds cannot quietly grow either.
#[test]
fn cluster_manager_surplus_is_declared() {
    ...
    assert_eq!(
        surplus,
        to_set(CLUSTER_ADDITIONS),
        "the clustered path's additions over upstream have drifted from \
         CLUSTER_ADDITIONS. If cluster added a call, list it there with a \
