...
//! That stands for response cursors and the request journal, which is upstream's
//! own, per node (D-74). It is **superseded for flow state** (issue #16, Gap E):
//! a scenario mid-flight across a fleet is the one piece of runtime state whose
//! loss is indistinguishable, to the test being run, from the product
//! misbehaving. A full-cluster restart that resets every flow to step one fails
//! the test suite that was using it.
//!
