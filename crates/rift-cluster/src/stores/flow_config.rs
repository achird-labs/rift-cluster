    ...
    pub fn from_imposter(config: &ImposterConfig) -> Result<Self, String> {
        ...
        let scope = match flow_state.extra.get(KEY_CONTEXT_SCOPE) {
            ...
            Some(value) => match value.as_str() {
                ...
                // Refused by name rather than aliased to `imposter` or `fleet`.
                // Tenancy left the fleet with #550, and either alias silently
                // changes which imposters share a namespace — the one thing this
                // key exists to decide. An operator whose config declared a tenant
                // namespace must learn the namespace is gone, not discover it from
                // a session that stopped being shared.
                Some("tenant") => {
                    return Err(format!(
                        "flowState.{KEY_CONTEXT_SCOPE}: \"tenant\" no longer exists — the fleet \
                         has no tenants since #550. Use \"imposter\" (per-imposter namespace) or \
                         \"fleet\" (one namespace for the whole fleet)."
                    ));
                }
                _ => {
                    return Err(format!(
...
mod tests {
    ...
    fn an_unknown_context_scope_is_refused_naming_the_key_and_the_accepted_set() {
        ...
        assert!(!err.contains("tenant"), "tenancy left with #550: {err}");
    }

    /// #550: `tenant` is refused by name, never aliased. A config that declared a
    /// tenant namespace must fail loudly — silently folding it into `imposter` or
    /// `fleet` would change which imposters share flow state, which is the one
    /// thing this key decides.
    #[test]
    fn the_tenant_scope_is_refused_by_name_and_points_at_the_issue() {
        let err = FlowConfig::validate(&imposter(serde_json::json!({ "contextScope": "tenant" })))
            .expect_err("the tenant scope no longer exists");
        assert!(err.contains("contextScope"), "{err}");
        assert!(err.contains("tenant"), "{err}");
        assert!(
            err.contains("#550"),
            "the refusal names the issue that removed it: {err}"
        );
        assert!(
            err.contains("\"imposter\"") && err.contains("\"fleet\""),
            "and names what is left: {err}"
        );
    }

    /// The two namespaces are disjoint by construction, including for a caller-chosen
    /// flow id shaped like the other scope's prefix (RFC-005 §3.5's G5).
    #[test]
    fn the_scope_prefixes_are_disjoint_even_for_prefix_shaped_ids() {
        assert_eq!(ContextScope::Imposter.prefix_for(Some(6400)), "i6400:");
        assert_eq!(ContextScope::Fleet.prefix_for(Some(6400)), "f:");

        // Adversarial: an id that spells the other scope's prefix cannot land in it.
        let imposter_shaped = ContextScope::Imposter.scoped_flow_id(Some(6400), "f:x");
        let fleet_shaped = ContextScope::Fleet.scoped_flow_id(Some(6400), "i6400:x");
        assert_eq!(imposter_shaped, "i6400:f:x");
        assert_eq!(fleet_shaped, "f:i6400:x");
        assert_ne!(imposter_shaped, fleet_shaped);
    }
    ...
    fn same_named_flows_share_one_ring_key_only_under_fleet() {
        let a = ContextScope::Imposter.scoped_flow_id(Some(6400), "checkout-77");
        let b = ContextScope::Imposter.scoped_flow_id(Some(6401), "checkout-77");
        assert_eq!(a, "i6400:checkout-77");
        assert_eq!(b, "i6401:checkout-77");
        assert_ne!(a, b, "imposter scope must keep same-named flows apart");

        let fa = ContextScope::Fleet.scoped_flow_id(Some(6400), "checkout-77");
        let fb = ContextScope::Fleet.scoped_flow_id(Some(6401), "checkout-77");
