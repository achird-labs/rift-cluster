...
function CreateTenantForm({
  ...
  const submit = (event: FormEvent): void => {
    ...
    onSubmit({
      id,
      displayName,
      // Every quota field, always: none of them has a serde default on the server, so a
      // present-but-partial object is a `missing field` parse error and a flat 400. These are the
      // server's own defaults (`Quotas::default`).
      quotas: {
        maxImposters: 1000,
