...
describe("sign out", () => {
  it("asks the server to end the session rather than only forgetting it locally", async () => {
    ...
    // made still carried the previous session — signed out in appearance only.
  ...
  it("removes the previous session's cached reads rather than refetching them", async () => {
    /*
     * Removal, not invalidation. Invalidation keeps the old data readable while the refetches are
     * in flight, so the previous session's imposters and routes would stay on screen for whoever
     * signs in next.
     *
     * The session probe is the deliberate exception and is reset instead, so the mounted observer
     * refetches and its 401 moves the console to the login screen — see
     * `signout-redirect.test.tsx`, which covers that end of it. This test owns the other end.
     */
    const client = new QueryClient();
    client.setQueryData(["imposters"], [{ port: 4545, name: "someone-elses" }]);
    client.setQueryData(["front-door-routes"], [{ id: "edge" }]);
    stubFetch({ "/session": { status: 204 } });
    renderInApp(<SignOut />, { client });

    await userEvent.setup().click(screen.getByTestId("sign-out"));

    await waitFor(() => expect(client.getQueryData(["imposters"])).toBeUndefined());
    expect(client.getQueryData(["front-door-routes"])).toBeUndefined();
  });
