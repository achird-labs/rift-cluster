...
export interface components {
    schemas: {
        ...
        QuotaUsage: {
             ...
             *
             *     **Fleet-scoped flows are counted against no tenant.** A flow under `contextScope: "fleet"` is shared by construction, so charging it to every tenant would double-count it and charging it to one would be arbitrary. Only imposter-scoped flows, on ports this tenant owns, are counted here. Tenant-scoped flows (`contextScope: "tenant"`, #288) are attributable — their key names the tenant — but the count fan-out is keyed by port and does not yet include them (issue #413); until it does, a tenant using `tenant` scope reads a floor here.
             *
