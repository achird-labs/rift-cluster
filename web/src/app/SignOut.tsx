...
import { SESSION_KEY } from "./session.tsx";
 ...
 * session is to expire it or clear site data by hand.
 *
...
export function SignOut(): ReactNode {
  ...
  async function signOut(): Promise<void> {
    ...
    try {
       ...
       *
       * First drop every read *except* the session probe, so the previous session's imposters and
       * route table are gone rather than merely stale — invalidation would leave them on screen for
       * whoever signs in next, for as long as the refetches take.
       *
       * Then reset the probe specifically, which refetches it because `App` still has it mounted.
       ...
       * so nothing refetches and the console keeps rendering the signed-out shell. That bug shipped
       * and was caught by hand, because the test asserted the cache had emptied instead of
       * asserting the operator reached the login screen — `signout-redirect.test.tsx` now pins the
