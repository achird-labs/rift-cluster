...
| every `Amends:` in the register has an `Amended by D-n` callout in the amended section | error | the spec must announce, *at the section a reader lands on*, that it no longer holds. Four verbs are defined — `Amended`, `Superseded`, `Reversed`, `Retired by` — and the checker reads only those |
