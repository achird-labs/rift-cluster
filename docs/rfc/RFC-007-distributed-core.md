...
1. **Should `POST /session` survive?** With one API key the console could send the key on every
   ...
   **Resolved by #550 (D-73):** kept, simplified. The default was taken, and a second reason
   ...
   so dropping the cookie would have left none. (`POST /session/rotate` triggers it, and a changed
   `--api-key` is a second bound: D-85, #619.) The payload's subject is now the constant
