...
| `--cluster-seeds <ADDR[,ADDR...]>` | Existing members to join through. Re-resolved on every attempt, so a headless service that gains members is picked up |
...
| `--cluster-degraded-mode <reject\|local>` | What to do with a config write whose owner is unreachable (default `reject`) |
