...
use cluster_chaos::{
    ...
    FRONT_DOOR_HOST_PORTS, FRONT_PORT, NODES, PROXY_ORIGIN_ADMIN_PORT, PULL_ON_MISS_HOST_PORTS,
...
/// (issue #228). An untargeted build takes the Dockerfile's *last* stage, and
/// which stage that is is not a stable fact: from #228 until #552 it was the
/// chaos suite's clock-lying `runtime-faketime`, so an untargeted build
/// silently produced that instead of the production image — a flip that
/// actually shipped once, one file away from the compose that was fixed. The
/// stage ordering has since changed back, which is exactly why this stays: the
/// guard is what makes appending a stage safe, in either direction.
///
    ...
    for path in files {
        ...
        assert!(
            ...
            "{}: {builds} build(s) against deploy/Dockerfile but only {targets} `target:` \
             pin(s) — an untargeted build takes whichever stage is last in the \
             Dockerfile, which is not this file's to assume",
            path.display()
        );
    }
}
...
/// why. 6520 continues the numbering C17/C18 (6500-6512) already use in
...
async fn c19_front_door_routes_around_bind_divergence() {
    ...
    loop {
        if fleet_members(NODES[1].admin).await.is_ok_and(|body| {
            body["bind_failures"]
                .get(&squatted)
...
/// used to carry twice over, once each for C18/C26, now consolidated
...
/// re-committed an unchanged config at a new revision fails here too.
#[tokio::test]
#[ignore = "needs a container runtime"]
async fn c26_replicated_imposters_survive_a_full_cluster_restart_by_snapshot_install() {
    ...
    //
    // Against an open admin plane, like every other scenario in this tier: the fleet boots with no
    // `MB_APIKEY`, so no credential is presented anywhere below. This scenario is about the
    // snapshot and cold-start paths, and leaving the plane open keeps that the only variable
    // under test.
    let mut ports: Vec<u16> = Vec::new();
    for (i, node) in NODES.iter().enumerate() {
        let port = C26_FIRST_PORT + i as u16;
        c26_write_imposter(node, port, &format!("seed-{i}")).await;
        wait_converged(u64::from(port), CONVERGE_TIMEOUT)
            .await
            .expect("converged");
        ports.push(port);
    ...
    let baseline = c26_imposters_on_every_node(&ports).await;
    assert_eq!(
        baseline[0].len(),
        ports.len(),
    ...
    for i in 0..IMPOSTERS_WHILE_DOWN {
        ...
        c26_write_imposter(via, port, &format!("while-down-{i}")).await;
        ports.push(port);
    }
    ...
    for &port in &ports {
        cluster_chaos::wait_converged_on(&live, u64::from(port), CONVERGE_TIMEOUT)
            .await
            .unwrap_or_else(|e| panic!("imposter {port} never reached both live nodes: {e}"));
    }
    let live_rows_0 = c26_imposter_rows(live[0], &ports).await;
    let live_rows_1 = c26_imposter_rows(live[1], &ports).await;
    assert_eq!(
        live_rows_0.len(),
        ports.len(),
    ...
    wait_admin_reachable(lagging.admin, Duration::from_secs(120))
        .await
    ...
    let caught_up = loop {
        let rows = c26_imposter_rows(lagging, &ports).await;
        if rows == live_rows_0 {
            break rows;
        }
        if std::time::Instant::now() >= deadline {
    ...
    let before = c26_imposters_on_every_node(&ports).await;
    assert_eq!(
        before[0].len(),
        ports.len(),
    ...
    let after = c26_imposters_on_every_node(&ports).await;
...
async fn c26_write_imposter(node: &cluster_chaos::Node, port: u16, marker: &str) {
    let (status, body) = put_imposter_config(
        node.admin,
        &serde_json::json!({
            "port": port,
...
/// carries (it ends `@<revision>`; `admin_front::imposter_read_token`),
...
/// node's own admin authority — `http://127.0.0.1:<that node's admin port>/...`,
/// a self-reference and not a divergence. Everything else in a stub is
/// replicated content.
///
...
async fn c26_imposters_on_every_node(ports: &[u16]) -> Vec<Vec<(u16, u64, serde_json::Value)>> {
    let mut out = Vec::with_capacity(NODES.len());
    for node in &NODES {
        out.push(c26_imposter_rows(node, ports).await);
    }
    out
}

/// The proxy-recording fixture (#228, C10/C11): a counting origin on the
/// standalone `proxy-origin` server (in-network by service name, its own
/// single-node admin published at 46525 — a reserved port), plus a proxyOnce
/// and a proxyAlways imposter on the fleet routed through the front doors.
...
const PROXY_ORIGIN_ADMIN: u16 = PROXY_ORIGIN_ADMIN_PORT;
const PROXY_ORIGIN_URL: &str = "http://proxy-origin:6810";
...
///
/// **Every assertion here is per node, and stays true under D-74 (#552).** What
/// is compared across the fleet is each node's own *applied stub config*
/// ([`recorded_stub_paths`]) — replicated state, which is exactly the thing
/// that must agree — and the origin's own call counts, read from the
/// standalone un-clustered `proxy-origin` server's single-node journal. Neither
/// is a clustered recorded-request read, so removing the fleet journal merge
/// takes nothing away from this scenario: "no recording is lost" was always
/// asserted through consensus state and the upstream's own tally, never
/// through a merged `savedRequests` view.
#[tokio::test]
#[ignore = "needs a container runtime"]
...
/// rebound. None of them says the returned owner is being **reached** for the
/// cursor key, which is what Phase 3 goes on to assert. Between the last of
/// those waits and the first sprayed decision there is a window in which every
/// node is healthy and some callers still will not route to the owner, so their
/// decisions degrade — correctly, per D-47 — and the assertion fails for a
/// reason the scenario is not about.
///
...
/// and an unapplied port defaults to `Local`
/// (`crates/rift-cluster/src/stores/sequencer.rs`) — a path that sets no
/// annotation and increments no counter. A returned owner whose registry has
/// not re-synced therefore answers from a fresh per-process cursor, silently,
/// and only the *body sequence* can tell. So every probe's body is returned and
/// folded into the caller's cycling assertion; swallowing them here would trade
...
async fn c33_owner_mode_sequencing_cycles_fleet_wide_and_degrades_on_owner_kill() {
    ...
    // owner being *reached* for the cursor key, which is what the assertion
    ...
    let ready = c33_wait_ring_answers(&all, C33_RING_RETAKE_BOUND)
        .await
