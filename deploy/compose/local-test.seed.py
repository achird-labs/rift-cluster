...
def check() -> None:
    ...
    want = [CATALOG, ORDERS, PAYMENTS, EDGE, LEDGER]
    if ports != want:
    ...
    # One data-plane request per node, through each node's own imposter port,
    # to show the imposters really were materialised on all three.
    for label, port in (("node1", 14545), ("node2", 24545), ("node3", 34545)):
        try:
            with urllib.request.urlopen(
                f"http://localhost:{port}/products", timeout=10
            ) as r:
                if r.status != 200:
                    failures.append(f"{label} /products -> {r.status}")
        except Exception as e:
            failures.append(f"{label} /products: {e}")

    # And one through the front door, which is the replicated route table doing
    # the dispatch rather than anything Docker published.
    try:
        with urllib.request.urlopen(FRONT + "/catalog/products", timeout=10) as r:
            if r.status != 200:
                failures.append(f"front door /catalog/products -> {r.status}")
    except Exception as e:
        failures.append(f"front door /catalog/products: {e}")
...
def main() -> int:
    ...
    if failures:
        ...
        for f in failures:
            print(f"  - {f}")
        return 1
    print("OK — 5 imposters and 5 routes, replicated and serving on all three nodes")
    return 0
